...
func NewRouteFileParser() *RouteFileParser {
	return &RouteFileParser{}
}

func (r *RouteFileParser) Parse(f io.Reader) ([]domain.Point, error) {
	buf := &bytes.Buffer{}
...
func (r *RouteFileParser) toPoint(gpxPoint gpx.GPXPoint) (domain.Point, error) {
	...
	altitude := domain.NewAltitude(gpxPoint.Elevation.Value())
